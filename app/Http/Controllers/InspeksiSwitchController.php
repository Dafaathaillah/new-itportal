...
class InspeksiSwitchController extends Controller
{
    public function ping()
    {
        ...
        $ips_inventory = InspeksiSwitch::where('month', $month)->where('year', $year)->where('device_status', 'OFFLINE')->orWhere('device_status', null)->get();
        $ips = [];
        ...
        foreach ($ips_inventory as $ipPing) {
            ...
            if ($status === 0) {
                ...
                $data['updateInspeksi'] = InspeksiSwitch::firstWhere('id', $ipPing->id)->update($data['dataInspection']);
            } else {
                $data['dataInspection'] = [
                    // 'condition' => 'BAGUS',
    ...
    {
        $id = $request->input('id');
        $ip = InspeksiSwitch::where('id', $id)->first();
        ...
        if ($status === 0) {
            ...
            $data['updateInspeksi'] = InspeksiSwitch::firstWhere('id', $id)->update($data['dataInspection']);
        } else {
            $data['dataInspection'] = [
                // 'condition' => 'BAGUS',
                'inspection_status' => 'belum_inspeksi',
    ...
    {
        $id = $request->id;
        $dataWirelless = InspeksiSwitch::where('id', $id)->first();
        ...
        if ($request->condition == 'RUSAK' || $request->condition == 'SCRAP') {
            ...
            $data['updateInspeksi'] = InspeksiSwitch::firstWhere('id', $id)->update($data['dataInspectionEdit']);
            $data['updateInventorySwitch'] = InvSwitch::firstWhere('id', $dataWirelless->inv_switch_id)->update($data['dataInspectionEditInventory']);
    ...
    {
        $dataCheckStatusInspeksi = InspeksiSwitch::where('id', $request->id)->value('inspection_status');
        if ($dataCheckStatusInspeksi == 'sudah_inspeksi') {
            ...
            $data['udpateInspeksiApproval'] = InspeksiSwitch::firstWhere('id', $request->id)->update($dataApproveal);
            return response()->json($data);
        }else{
    ...
    public function approvalAll(Request $request)
    {
        ...
        InspeksiSwitch::where('month', $month)
            ->where('year', $year)
