...
class IctDeveloperController extends Controller
{
    public function login(Request $request)
    {
        ...
        $user = User::where('nrp', $request->nrp)->where('role', 'ict_developer')->first();
        ...
        if (empty($user)) {
            ...
            if (!$user || !Hash::check($request->password, $user->password)) {
                ...
                // return redirect()->route('login')https://firebase.google.com/?hl=id
                // ->with('message', 'user not defined your role is: ' . $userGet->role);
                if (empty($user)) {
                    return response()->json(['message', 'user not defined your role is: ' . $userGet->role . ', please change youre login page']);
                } else {
                    return response()->json([
                        'user' => $user,
                        'token' => $user->createToken('mobile', ['role:ict_developer'])->plainTextToken
                    ]);
                }
            }
        }
