...
class IctSectionHeadController extends Controller
{
    public function login(Request $request)
    {
        ...
        $user = User::where('nrp', $request->nrp)->where('role', 'ict_section_head')->first();
            ...
            if (!$user || !Hash::check($request->password, $user->password)) {
                ...
                if (empty($user)) {
                    ...
                    return response()->json([
                        'user' => $user,
                        'token' => $user->createToken('mobile', ['role:ict_section_head'])->plainTextToken
                    ]);
                }
            }
        }
