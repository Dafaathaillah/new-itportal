...
class InspeksiWirellessController extends Controller
{
    public function ping()
    {
        ...
        $ips_inventory = InspeksiWirelless::where('month', $month)->where('year', $year)->where('device_status', 'OFFLINE')->orWhere('device_status', null)->get();
        $ips = [];
        ...
        foreach ($ips_inventory as $ipPing) {
            ...
            if ($status === 0) {
                ...
                $data['updateInspeksi'] = InspeksiWirelless::firstWhere('id', $ipPing->id)->update($data['dataInspection']);
            } else {
                $data['dataInspection'] = [
                    // 'condition' => 'BAGUS',
    ...
    {
        $id = $request->input('id');
        $ip = InspeksiWirelless::where('id', $id)->first();
        ...
        if ($status === 0) {
            ...
            $data['updateInspeksi'] = InspeksiWirelless::firstWhere('id', $id)->update($data['dataInspection']);
        } else {
            $data['dataInspection'] = [
                // 'condition' => 'BAGUS',
                'inspection_status' => 'belum_inspeksi',
    ...
    {
        $id = $request->id;
        $dataWirelless = InspeksiWirelless::where('id', $id)->first();
        ...
        $maxId = InspeksiWirelless::max('id');

        if (is_null($maxId)) {
            $maxId = 0;
        }

        $uniqueString = 'PICA/BB/' . $year . '/' . str_pad(($maxId % 10000) + 1, 2, '0', STR_PAD_LEFT);
        ...
        if ($request->condition == 'RUSAK' || $request->condition == 'SCRAP') {
            ...
            $data['updateInspeksi'] = InspeksiWirelless::firstWhere('id', $id)->update($data['dataInspectionEdit']);
            $data['updateInventoryWirelless'] = InvWirelless::firstWhere('id', $dataWirelless->inv_wirelless_id)->update($data['dataInspectionEditInventory']);
    ...
    {
        $dataCheckStatusInspeksi = InspeksiWirelless::where('id', $request->id)->value('inspection_status');
        if ($dataCheckStatusInspeksi == 'sudah_inspeksi') {
            ...
            $data['udpateInspeksiApproval'] = InspeksiWirelless::firstWhere('id', $request->id)->update($dataApproveal);
            return response()->json($data);
        }else{
    ...
    public function approvalAll(Request $request)
    {
        ...
        InspeksiWirelless::where('month', $month)
            ->where('year', $year)
